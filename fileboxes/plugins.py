...
class ProjectAppPlugin(ProjectAppPluginPoint):
    ...
    def search(self, search_terms, user, search_type=None, keywords=None):
        ...
        if not search_type:
            file_boxes = FileBox.objects.find(search_terms, keywords).filter(project__in=projects)
            items = list(file_boxes)
            items.sort(key=lambda x: x.title.lower())
        elif search_type == "filebox":
            items = FileBox.objects.find(search_terms, keywords).filter(project__in=projects)
